...
function generatePDF(result: StackRecommendation, budget: number) {
  ...
  doc.setFontSize(22);
  doc.setFont("helvetica", "bold");
  doc.text("AI Stack Business Plan", margin, y);
  y += 10;
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  ...
  doc.text("Generated by AIStack Builder — tool prices are estimates.", margin, y);

  doc.save(`AIStack-Business-Plan-${analysis.businessType.replace(/\s+/g, "-")}.pdf`);
}
