...
const SUB_ICONS = [Megaphone, TrendingUp, UserSearch];
...
export function GrowthMarketing({ sections, tools }: Props) {
  return (
    <div className="space-y-6">
      {sections.map((section, idx) => {
        const Icon = SUB_ICONS[idx] || Megaphone;
        return (
              ...
              <div className="space-y-2">
                ...
                {section.actions.map((action, i) => (
                  <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-lg bg-background/30 border border-border/20">
                    <span className="flex items-center justify-center w-5 h-5 rounded-full bg-pink-500/10 text-pink-400 text-[10px] font-bold font-body shrink-0 mt-0.5">
                      {i + 1}
                    </span>
      ...
      {tools.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wide mb-3 font-body flex items-center gap-2">
            <Zap className="h-4 w-4 text-pink-400" />Tools Powering Growth
          </h3>
          <div className="grid gap-2 sm:grid-cols-2">
            {tools.map((tool) => (
              <PanelToolCard key={tool.id} tool={tool} accentColor="pink" />
            ))}
          </div>
        </div>
      )}
